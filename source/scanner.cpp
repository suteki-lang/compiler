...
// Set the source
void Scanner::set(std::string _source)
{
    source = _source;
    start  = 0;
    end    = 0;
...
{
    current.kind   = kind;
    current.data   = source.substr(start, end - start);
    current.line   = line;
    current.column = column;
...
// Make error token
TokenKind Scanner::make_token(std::string message)
{
...
TokenKind Scanner::make_identifier_token(void)
{
    ...
    // Find keyword
    std::string keyword = source.substr(start, end - start);

    if (keywords.find(keyword) != keywords.end())
        return make_token(keywords[keyword], keyword);

    return make_token(TokenKind::Identifier, keyword);
}
...
TokenKind Scanner::make_number_token(void)
{
    ...
    std::string number_string = source.substr(start, end - start);
    return make_token(TokenKind::Number, std::stod(number_string));
}
