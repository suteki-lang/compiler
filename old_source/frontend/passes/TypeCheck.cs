...
namespace Suteki
{
    ...
    partial class NodeCall : Node
    {
        public override ExpressionKind TypeCheck(Input input)
        {
            ...
            {
                input.Logger.Error("Function call parameter(s) count does not match function parameter(s) count.");
                return ExpressionKind.Void;
            }
            ...
            for (int index = 0; index < node.Parameters.Count; ++index)
            {
                ...
                if (!Type.Compare(parameter, expression))
                    input.Logger.Error($"Function call parameter ({index}) type does not match function parameter type.");
            }
